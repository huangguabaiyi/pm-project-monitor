import { DownloadCloud,RefreshCw,Trash2,UploadCloud } from 'lucide-react'
import { ChangeEvent,useEffect,useRef,useState } from 'react'
import { api } from '../api'
import { Field,Loading,PageHead,Toast } from '../components'
import type { DataMaintenanceResult,DeploymentUpdateStatus } from '../types'

export default function SystemMaintenance(){
  return <>
    <PageHead eyebrow="系统维护" title="维护" description="处理服务器更新、数据备份、迁移导入和清空操作。"/>
    <DeploymentUpdater/>
    <DataMaintenance/>
  </>
}
...
function DataMaintenance(){
  ...
  async function upload(event:ChangeEvent<HTMLInputElement>){
    ...
    try{
      ...
      setLastResult(result);setMessage('备份已导入')
    }catch(err){setError((err as Error).message)}
    finally{setBusy(false)}
  }
  async function clear(){
    ...
    try{
      ...
      setLastResult(result);setMessage('数据已清空')
    }catch(err){setError((err as Error).message)}
    finally{setBusy(false)}
  }
