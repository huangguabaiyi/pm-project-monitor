...
function Templates(){
  ...
  const unused=useMemo(()=>definitions.filter(d=>d.active&&!detail?.nodes?.some(n=>n.definition_id===d.id)),[definitions,detail])
  ...
  async function addNode(definition:Definition){if(!selected)return;try{await api.post(`/templates/${selected}/nodes`,{definition_id:definition.id,position_x:120+(nodes.length%3)*260,position_y:90+Math.floor(nodes.length/3)*180});setMessage('节点已加入画布');setError('');await refreshTemplate()}catch(err){setError((err as Error).message)}}
