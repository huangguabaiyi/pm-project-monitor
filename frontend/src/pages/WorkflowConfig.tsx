...
import { Copy,Edit3,GitFork,Layers3,Link2,Plus,Save,Trash2,X } from 'lucide-react'
...
type PaletteNodeData={node:TemplateNode}
const domainNames=(domains?:Domain[],fallback?:Domain)=>domains?.length?domains.map(domain=>domain.name).join('、'):fallback?.name||'未分组'
function PaletteNode({data}:{data:PaletteNodeData}){const n=data.node;return <div className="template-node" style={{'--domain':n.domain.color} as React.CSSProperties}><Handle type="target" position={Position.Left} onClick={event=>event.stopPropagation()}/><span>{domainNames(n.domains,n.domain)}</span><strong>{n.name}</strong><small>{n.completion_criteria||'未设置完成标准'}</small><Handle type="source" position={Position.Right} onClick={event=>event.stopPropagation()}/></div>}
...
function Templates(){
  ...
  const selectedEdgeId=selectedNodeIds.length===0?edges.find(edge=>edge.selected)?.id:undefined
  ...
  async function copyCurrent(){if(!detail)return;try{const copied=await api.post<Template>(`/templates/${detail.id}/copy`,{});await loadLists();setSelected(copied.id);setMessage(`已复制为「${copied.name}」`);setError('')}catch(err){setError((err as Error).message)}}
  async function deleteCurrent(){if(!detail||!window.confirm(`确定删除模板「${detail.name}」吗？模板节点和连线会一起删除，已创建的需求不受影响。`))return;try{const deletedName=detail.name;await api.del(`/templates/${detail.id}`);const remaining=await api.get<Template[]>('/templates');setTemplates(remaining);setSelected(remaining[0]?.id);if(!remaining.length){setDetail(undefined);setNodes([]);setEdges([])}setEditingTemplate(false);setMessage(`模板「${deletedName}」已删除`);setError('')}catch(err){setError((err as Error).message)}}
  ...
  async function moved(_:unknown,node:Node,movedNodes:Node[]=[]){const changed=movedNodes.length?movedNodes:[node];try{await Promise.all(changed.map(item=>api.patch(`/template-nodes/${item.id}`,{position_x:item.position.x,position_y:item.position.y})));const positions=new Map(changed.map(item=>[item.id,item.position]));setDetail(current=>current?{...current,nodes:current.nodes?.map(item=>positions.has(item.id)?{...item,position:positions.get(item.id)!}:item)}:current);setError('')}catch(err){setError((err as Error).message);await loadDetail()}}
  function clearSelection(){setNodes(current=>current.map(node=>node.selected?{...node,selected:false}:node));setEdges(current=>current.map(edge=>edge.selected?{...edge,selected:false}:edge))}
  async function deleteSelected(){if(!selectedNodeIds.length&&!selectedEdgeId)return;if(selectedNodeIds.length&&!window.confirm(`确定从模板中删除选中的 ${selectedNodeIds.length} 个节点吗？相关连线也会被删除。`))return;try{if(selectedNodeIds.length){for(const nodeId of selectedNodeIds)await api.del(`/template-nodes/${nodeId}`)}else if(selectedEdgeId){await api.del(`/template-edges/${selectedEdgeId}`)}const count=selectedNodeIds.length;clearSelection();setMessage(count?`已删除 ${count} 个模板节点`:'已删除所选连线');setError('');await refreshTemplate()}catch(err){setError((err as Error).message)}}
  if(!templates)return <Loading/>
  return <div className="template-layout"><aside className="panel template-sidebar"><form onSubmit={create}><h3>模板</h3><div className="inline-create"><input name="name" required placeholder="新模板名称"/><button className="icon"><Plus/></button></div></form><div className="template-list">{templates.map(t=><button className={selected===t.id?'active':''} key={t.id} onClick={()=>setSelected(t.id)}><strong>{t.name}</strong><span>{t.node_count} 节点 · {t.edge_count} 连线</span></button>)}</div><hr/><h3>可用节点</h3><p className="sidebar-hint">点击添加到当前模板</p><div className="palette">{unused.map(d=><button onClick={()=>addNode(d)} key={d.id}><i style={{background:d.domain.color}}/><span><strong>{d.name}</strong><small>{d.domain.name}</small></span><Plus/></button>)}</div></aside><section className="panel template-workspace"><div className="section-head"><div><h2>{detail?.name||'选择一个模板'}</h2><p>点击选择，按住 Command/Ctrl 多选；按住 Shift 拖动空白区域可框选。</p></div><div className="section-actions">{selectedNodeIds.length>0&&<span className="flow-selection-count">已选 {selectedNodeIds.length} 个节点</span>}<button className="button ghost compact" onClick={copyCurrent} disabled={!detail}><Copy size={15}/>复制模板</button><button className="button ghost compact" onClick={()=>setEditingTemplate(true)} disabled={!detail}><Edit3 size={15}/>编辑模板</button><button className="button danger compact" onClick={deleteCurrent} disabled={!detail}><Trash2 size={15}/>删除模板</button><button className="button danger compact" onClick={deleteSelected} disabled={!selectedNodeIds.length&&!selectedEdgeId}><Trash2 size={15}/>删除所选{selectedNodeIds.length>1?`（${selectedNodeIds.length}）`:''}</button></div></div>{editingTemplate&&detail&&<form className="template-edit-form" onSubmit={saveTemplate}><Field label="模板名称"><input name="name" required defaultValue={detail.name}/></Field><Field label="说明"><input name="description" defaultValue={detail.description}/></Field><label className="check"><input name="active" type="checkbox" defaultChecked={detail.active}/>使用中</label><div className="inline-actions"><button type="button" className="button ghost compact" onClick={()=>setEditingTemplate(false)}>取消</button><button className="button primary compact"><Save size={15}/>保存模板</button></div></form>}{selected?<div className="flow-canvas template-flow"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={connect} onNodeDragStop={moved} fitView deleteKeyCode={null} selectionOnDrag={false} selectNodesOnDrag={false} elementsSelectable connectOnClick={false} connectionDragThreshold={10} autoPanOnConnect={false} elevateNodesOnSelect={false} elevateEdgesOnSelect><Background gap={22} size={1}/><Controls showInteractive={false}/><MiniMap/></ReactFlow><div className="canvas-legend"><span><i className="serial"/>串行：依次连接</span><span><i className="parallel"/>并行：从同一节点分叉</span><span>禁止形成循环依赖</span></div></div>:<Empty title="还没有模板" detail="先在左侧创建一个模板。"/>}</section>{message&&<Toast text={message}/>} {error&&<Toast text={error} type="error"/>}</div>
}
